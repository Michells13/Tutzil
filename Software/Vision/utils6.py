...
#     end_time = time.time()
#     print(f"Time taken to send data: {end_time - start_time:.4f} seconds")
# =============================================================================

def enconde_data_to_json(all_data):
    
    data_json = json.dumps(all_data)
   # encoded_data= data_json.encode('utf-8')
   # encoded_data= json.dumps(data_json).encode('utf-8')
...
def convert_encoded_image_to_bytes(encoded_image):
    ...
    return string_data
def convert_raw_results_to_Bytes(results) :
    """
    Convierte resultados de deteccion a un stream de bytes.

    Args:
        results: Resultados del modelo.

    Returns:
        string_results: La imagen en formato de bytes.
    """
    data= np.array(results)
    string_results=data.tobytes()
    return string_results

def send_packet_over_tcp(connection, string_data, json_data):
    """
    Envía un paquete con : 1.-El tamano de longitud de la imagen
                           2.-La imagen
                           3.-El paquete Json de boxes, classes and ID's con el siguiente formato 
                              Tracking IDs: {
                                  'len': 2, 
                                  'track_ids': [1.0, 4.0], 
                                  'class_ids': [0.0, 0.0], 
                                  'normalized_boxes': [[0.00016787159256637096, 0.0135871060192585,
                                                        0.9260271787643433, 0.9996688961982727], 
                                                       [0.6212233304977417, 0.44897201657295227,
                                                        0.8073541522026062, 0.690412163734436]]}
           
    Args:
        connection: El socket de la conexión TCP/IP.
        string_data: La imagen en formato de bytes.
        json_data: El paquete Json con bounding boxes, classes  and id's
        
    """
    # Convert the string data to bytes (if not already in bytes format)
    #string_data_bytes = string_data.encode('utf-8') if isinstance(string_data, str) else string_data
    
    # Serialize json_data to a JSON string and then convert it to bytes
    start_time = time.time()

    json_data_bytes = json.dumps(json_data).encode('utf-8')
    end_time = time.time()
    print(f"Json to bytes: {end_time - start_time:.4f} seconds")

    
    # Pack the data sizes and concatenate the data
    start_time = time.time()
    packed_data = struct.pack(">L", len(string_data)) + string_data
    end_time = time.time()
    print(f"Packet image: {end_time - start_time:.4f} seconds")
    
    
    
    
    start_time = time.time()
    packed_data += struct.pack(">L", len(json_data_bytes)) + json_data_bytes
    end_time = time.time()
    print(f"Packet Json : {end_time - start_time:.4f} seconds")
    # Send the packed data over the connection
    
    start_time = time.time()
    connection.sendall(packed_data)
    end_time = time.time()
    print(f"Send Packets : {end_time - start_time:.4f} seconds")
...
def process_frame(cap, model, tracker, traclet, display_traclets, Show, Stream, Verbose, Heatmap, heatmap_obj, conf, iou, persist, track_history):
    ...
    for result in results:
        ...
        if boxes.is_track:
            ...
            normalized_boxes= boxes.xyxyn.tolist()
        
        
            # Create a vector containing all the data
            data_vector = {
                "len": len(track_ids),
                "track_ids": track_ids,
                "class_ids": class_ids,
            ...
            all_data.append(data_vector)
            
    for data in all_data:
        print(f"Tracking IDs: {data}")
        # print(f"Class IDs: {data['class_ids']}")
        # print(f"Normalized Boxes: {data['normalized_boxes']}")
 
            
    if cv2.waitKey(25) & 0xFF == ord('q'):
        return False, frame, results

    return True, frame, results, all_data



def start_server(model,device,config,camDevice):
# =============================================================================
            ...
            try:
                ...
                while True:
                    ...
                    if decoded_data == "0000":
                        ...
                        loop_status, frame, results, json_data=process_frame(cap, model, config["tracker"],config["traclet"] ,config["display_traclets"] , config["Show"],config["Stream"],config["Verbose"] ,config["Heatmap"], heatmap_obj, config["conf"], config["iou"],config["persist"] , track_history)
                        end_time = time.time()
                        print(f"Proccess frame: {end_time - start_time:.4f} seconds")
                        ...
                        end_time = time.time()
                        print(f"Encode Image to  jpeg: {end_time - start_time:.4f} seconds")
                        ...
                        end_time = time.time()
                        print(f"Encode Image to bytes: {end_time - start_time:.4f} seconds")
                        ...
                        if cnt1>100:
                            ...
                            send_packet_over_tcp(connection, string_data, json_data)
                            #send_image_over_tcp(connection, string_data, json_data)
